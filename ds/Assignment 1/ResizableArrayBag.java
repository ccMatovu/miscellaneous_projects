...
public final class ResizableArrayBag<T> implements BagInterface<T>
{
   ...
   private void checkintegrity()
   {
	...
	/**Adds together the contents of two bags.
	 * @param bag
	 * @return a bag that contains contains both the contents of the calling bag and the bag passed as a parameter.
	 */
	public BagInterface<T> union(BagInterface<T> bag) {
		//setting the size of the union bag by combining the size of two bags to be combined and creating it.
		int unionSize = this.getCurrentSize() + bag.getCurrentSize();
		ResizableArrayBag<T> unionBag = new ResizableArrayBag<>(unionSize);

		//getting the cotents of the calling bag and putting them into the union bag
		T[] contents = this.toArray();
		for(int i=0;i<this.getCurrentSize();i++){
			unionBag.add(contents[i]);
		}

		//getting the contents of the bag passed as a parameter and putting them into the union bag
		contents = bag.toArray();
